...
export default function OutputPanel({ jsonValue, fileName, packs = [], showDiff = false }: OutputPanelProps) {
    ...
    <div className="border border-border bg-muted/20 rounded-xl p-4 flex flex-col h-full space-y-4 shadow-sm">
      <div className="flex items-center justify-between">
        <div className="flex flex-col">
          <span className="text-[10px] text-foreground/50 font-bold uppercase tracking-wider">Output File</span>
          <span className="text-sm font-bold font-mono text-primary">{fileName}</span>
        </div>
      ...
      <div className="flex-1 bg-white dark:bg-[#050505] border border-border rounded-xl p-3 overflow-auto max-h-[350px] min-h-[200px] flex flex-col">
        {activeTab === 'raw' || !showDiff || packs.length === 0 ? (
          ...
          <div className="space-y-2 flex-1 text-xs">
            <h4 className="font-semibold text-foreground/60 border-b border-border pb-1">Perubahan Entry JSON:</h4>
              ...
              {packs.map((pack, idx) => (
                ...
                >
                  <div className="truncate flex-1">
                    ...
                    <span className="text-[10px] opacity-60 ml-2">v{pack.version.join('.')}</span>
                  </div>
